# argocd-mcp — dev shortcuts
BINARY      := argocd-mcp
PKG         := github.com/truepace-io-oss/argocd-mcp-server
VERSION     ?= dev
...
## E2E against a real kind cluster running Argo CD
test-e2e-kind:
...
helm-lint:
	helm lint deploy/helm/argocd-mcp
