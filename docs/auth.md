...
auth:
  ...
  static:
    ...
    tokens:
      - name: ci
        tokenFile: /etc/amcp/auth/ci-token   # preferred (ESO / rotatable)
      - name: laptop
...
{ "mcpServers": { "k8s": {
  ...
  "headers": { "Authorization": "Bearer ${AMCP_TOKEN}" }
}}}
```
...
auth:
  ...
  oidc:
    ...
    issuer: "https://authentik.example.com/application/o/argocd-mcp/"
    audience: "https://argocd-mcp.example.com"   # == token `aud` (RFC 8707)
...
sequenceDiagram
    ...
    participant M as argocd-mcp (Resource Server)
...
2. Set `include_claims_in_access_token: true` — the MCP validates the **access**
   ...
   `oidc.audience` to that `client_id` (e.g. `argocd-mcp`).
...
```json
{ "mcpServers": { "argocd-mcp": { "type": "http", "url": "https://argocd-mcp.example.com/mcp" } } }
```
...
```
Then trigger login: `claude mcp login argocd-mcp` (or `/mcp` → *Authenticate*);
reset a bad state with `claude mcp logout argocd-mcp`. Requires a recent
...
## Note: authentication vs Kubernetes RBAC
...
## The authenticated identity is attributed on Argo CD operations

When agent auth is enabled, the verified caller is written to
`operation.initiatedBy.username` on every `app_sync` and `app_rollback`, so the
Argo CD UI and `app_history` show *who* asked for a deployment:

| Auth mode | `initiatedBy.username` |
|-----------|------------------------|
| disabled | `argocd-mcp` |
| static token named `ci` | `argocd-mcp:static:ci` |
| OIDC | `argocd-mcp:<usernameClaim>`, e.g. `argocd-mcp:daniel` |

The OIDC value comes from `auth.oidc.usernameClaim` (default
`preferred_username`), falling back to the token subject. This is asserted
end-to-end in `test/e2e/e2e_auth_test.go` for both modes.
