...
// restConfigFor builds a *rest.Config for the Kubernetes API server that hosts
// one Argo CD instance, purely from ServiceAccount credentials. No custom
// authentication is performed: the resulting client sends a bearer token and the
// Kubernetes API server + RBAC authorize every request.
//
...
func restConfigFor(c config.InstanceConfig) (*rest.Config, error) {
	...
	case c.InCluster:
		...
		if err != nil {
			return nil, fmt.Errorf("instance %q: in-cluster config: %w", c.Name, err)
		}
	...
	case c.KubeconfigFile != "":
		...
		if err != nil {
			return nil, fmt.Errorf("instance %q: kubeconfig context %q: %w", c.Name, c.Context, err)
		}
	...
	default: // explicit remote: server + CA + token
		...
		case c.CertificateAuthorityData != "":
			...
			if derr != nil {
				return nil, fmt.Errorf("instance %q: certificateAuthorityData is not valid base64: %w", c.Name, derr)
			}
