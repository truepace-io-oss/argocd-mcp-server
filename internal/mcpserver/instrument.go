...
// instanceLabel returns the resolved instance name for a tool input (default
// when the caller omitted it; "-" for tools with no instance arg).
...
func classifyResult(res *mcp.CallToolResult, err error) string {
	...
	case strings.Contains(low, "forbidden"):
		return "forbidden"
	case strings.Contains(low, "already in progress"):
		return "conflict"
