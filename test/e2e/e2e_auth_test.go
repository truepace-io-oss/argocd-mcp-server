...
// startAuthMCP serves an MCP instance (targeting envtest with a sync-capable
// token) wrapped in the given auth config, mirroring main.go's wiring. Returns
// the base URL.
func startAuthMCP(t *testing.T, ctx context.Context, namespace, clusterToken string, authCfg config.Auth) string {
	t.Helper()
	cfg := &config.Config{
		LogLevel:        "error",
		DefaultInstance: "envtest",
...
// syncToken provisions a sync-capable SA in envtest, seeds one Application and
// returns the SA token.
func syncToken(t *testing.T, ns, sa, role string) string {
	cs := adminClient(t)
	ensureNamespace(t, cs, ns)
	seedApp(t, ns, "app", seedOpts{sync: "OutOfSync", health: "Healthy", revision: "old"})
	tok := mintToken(t, cs, ns, sa)
	grantArgoRole(t, cs, ns, sa, role, syncVerbs)
	return tok
}

func TestE2EAuthStatic(t *testing.T) {
	const ns = "e2e-auth-static"
	clusterTok := syncToken(t, ns, "mcp-sa", "e2e-auth-static-role")
	base := startAuthMCP(t, context.Background(), ns, clusterTok, config.Auth{
		Enabled: true,
	...
	if isErr || !strings.Contains(out, ns+"/app") {
		t.Fatalf("authenticated tool call failed: err=%v out=%s", isErr, out)
	}

	// The authenticated identity must reach the tool handler: a sync performed
	// through this session is attributed to the static token's name.
	if out, isErr := callText(t, sess, "app_sync", map[string]any{"name": "app"}); isErr {
		t.Fatalf("app_sync failed: %s", out)
	}
	user, _, _ := unstructured.NestedString(getApp(t, ns, "app").Object, "operation", "initiatedBy", "username")
	if user != "argocd-mcp:static:e2e" {
		t.Fatalf("operation.initiatedBy.username = %q, want the authenticated caller", user)
	}
}
...
func TestE2EAuthOIDC(t *testing.T) {
	...
	base := startAuthMCP(t, ctx, ns, clusterTok, config.Auth{
		Enabled: true,
	...
	tok := iss.mint(t, map[string]any{
		"iss": iss.url, "aud": aud, "sub": "u1", "preferred_username": "daniel",
	...
	t.Cleanup(func() { _ = sess.Close() })
	if out, isErr := callText(t, sess, "apps_list", map[string]any{}); isErr || !strings.Contains(out, ns+"/app") {
		t.Fatalf("OIDC tool call failed: err=%v out=%s", isErr, out)
	}

	// The OIDC username claim must be attributed on the Argo CD operation.
	if out, isErr := callText(t, sess, "app_sync", map[string]any{"name": "app"}); isErr {
		t.Fatalf("app_sync failed: %s", out)
	}
	user, _, _ := unstructured.NestedString(getApp(t, ns, "app").Object, "operation", "initiatedBy", "username")
	if user != "argocd-mcp:daniel" {
		t.Fatalf("operation.initiatedBy.username = %q, want argocd-mcp:daniel", user)
	}
